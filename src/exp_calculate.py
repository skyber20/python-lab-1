...
def func_calculate(exp_in_rpn: list[str]) -> int | float:
        ...
        if elem not in OPERATORS:
            ...
            if elem in '〜$':
                try:
                    ...
                    raise ArithmeticError(f"В стеке не нашлось числа для выполнения операции {elem}")
            else:
                try:
                    ...
                    raise ArithmeticError(f"В стеке не нашлось 2ух чисел для выполнения операции {elem}")
